...
def _call_groq(prompt: str, max_tokens: int = 512) -> str | None:
    """
    Groq wrapper using OpenAI-compatible endpoint.
...
def generate_response(email_obj: dict, analysis: dict) -> str:
    """
    Generate a context-aware draft reply using retrieved KB + Groq.
    Falls back to a heuristic reply if Groq is not available.
    """
    ...
    priority_line = (
        "I've marked this as urgent and escalated it to our team."
        if analysis.get("priority") == "Urgent"
        else "I've logged this request and our team will follow up."
    )
